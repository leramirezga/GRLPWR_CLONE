...
@section('title') Blogs {{$user->nombre}} @endsection
        ...
        @if($blogs->count()==0)
            <div class="margenes-normales floating-card bg-semi-transparent text-center">
                ...
                @if($user == \Illuminate\Support\Facades\Auth::user())
                    <button type="button" class="btn btn-success m-auto d-inline-block d-md-block" data-toggle="modal" data-target="#crearInfoBlogModal">Crear Blog</button>
                @endif
            </div>
                    ...
                    <div class="card floating-card bg-dark">
                        @if($blogs[$i]->tipo == \App\Utils\Constantes::BLOG_TIPO_BLOG)
                            <a href="/blog/{{$blogs[$i]->slug}}">
                                <img class="card-img-top" src="/images/blogs/{{$blogs[$i]->portada}}" style="height: 37.5vh;">
                            </a>
                        @endif
                        <a href="/blog/{{$blogs[$i]->slug}}">
                            <div class="card-body text-center d-flex" style="height: 12.5vh; padding:10px">
                                <span class="m-auto" style="overflow: hidden; white-space: nowrap; text-overflow: ellipsis; max-height: 100%; font-size: 24px">{{$blogs[$i]->titulo}}</span>
                            </div>
                        </a>
                    </div>
                </div>
